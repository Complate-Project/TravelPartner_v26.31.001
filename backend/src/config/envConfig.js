...
function validateEnv() {
    ...
    // Warn if ISMS SMS credentials are missing (non-blocking — SMS will just fail)
    if (!process.env.ISMS_API_TOKEN || !process.env.ISMS_SID) {
        if (isProduction) {
            console.warn("[config] WARNING: ISMS_API_TOKEN or ISMS_SID not set. Password-reset SMS will not work.");
        }
    }

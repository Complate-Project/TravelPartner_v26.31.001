...
function statusStyle(status: string): { label: string; color: string } {
    const map: Record<string, { label: string; color: string }> = {
        ...
        missed: { label: 'Missed', color: '#D4A72C' },
    ...
    return (
        <div style={{
            ...
            <div style={{ width: '100%', maxWidth: 480, margin: '0 auto', display: 'flex', flexDirection: 'column', height: '100%' }}>
                {/* Header */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: '1px solid var(--border-default)', flexShrink: 0 }}>
                    <button onClick={onClose} aria-label="Close"
                        style={{ width: 40, height: 40, borderRadius: 12, border: '1px solid var(--border-default)', background: 'var(--bg-input)', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, WebkitTapHighlightColor: 'transparent' }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6" /></svg>
                    </button>
                    <h1 style={{ color: 'var(--text-primary)', fontSize: '1.2rem', fontWeight: 700, margin: 0, letterSpacing: '-0.02em' }}>Call History</h1>
                    {total > 0 && <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem', marginLeft: 'auto' }}>{total}</span>}
                </div>
                ...
                <div style={{ padding: '10px 12px', flexShrink: 0 }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                        <input
                            ...
                            style={{
                                flex: 1, padding: '9px 12px', borderRadius: 10, border: '1px solid var(--border-default)',
                    ...
                    {FILTER_TABS.map(tab => (
                        <button key={tab.key} onClick={() => setFilter(tab.key)}
                            style={{
                                ...
                                background: 'transparent', color: filter === tab.key ? 'var(--text-primary)' : 'var(--text-secondary)',
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px' }}>
                    {loading && calls.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Loading...</div>
                    ) : calls.length === 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', gap: 12 }}>
                            <div style={{ width: 64, height: 64, borderRadius: 20, background: 'rgba(212,167,44,0.1)', border: '1px solid rgba(212,167,44,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(212,167,44,0.6)" strokeWidth="1.8">
                                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
                                </svg>
                            </div>
                            <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', textAlign: 'center' }}>No call history yet</p>
                        </div>
                    ) : (
                        <>
                            {calls.map((call) => {
                                ...
                                return (
                                    <div key={call.id} style={{
                                        ...
                                        padding: '14px 8px', borderBottom: '1px solid var(--border-subtle)',
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <p style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: '0.95rem', margin: '0 0 4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                {peerName}
                                            </p>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
                                                ...
                                                <span style={{ color: 'var(--text-secondary)', fontSize: '0.7rem', whiteSpace: 'nowrap' }}>Audio</span>
                                                {call.duration_seconds != null && call.status === 'connected' && (
                                                    <>
                                                        <span style={{ color: 'var(--text-muted)', fontSize: '0.55rem' }}>•</span>
                                                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.7rem', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                                        ...
                                        <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 8 }}>
                                            ...
                                            {(call as any).cost != null && (
                                                <p style={{ color: 'var(--text-secondary)', fontSize: '0.65rem', margin: '0 0 2px', whiteSpace: 'nowrap' }}>
                                                    <PointsDisplay amount={(call as any).cost} decimals={2} />
                                                </p>
                                            )}
                                            <p style={{ color: 'var(--text-muted)', fontSize: '0.6rem', margin: 0, whiteSpace: 'nowrap' }}>
                            ...
                            {totalPages > 1 && (
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, padding: '16px 8px' }}>
                                    <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
                                        style={{
                                            padding: '8px 18px', borderRadius: 10, border: '1px solid var(--border-default)',
                                            background: page <= 1 ? 'transparent' : 'var(--bg-input)',
                                    ...
                                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>{page} / {totalPages}</span>
                                    <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
                                        style={{
                                            padding: '8px 18px', borderRadius: 10, border: '1px solid var(--border-default)',
                                            background: page >= totalPages ? 'transparent' : 'var(--bg-input)',

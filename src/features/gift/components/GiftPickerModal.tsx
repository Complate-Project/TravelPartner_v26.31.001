            ...
            {confirmGift && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h3 style={{ color: 'var(--text-primary)', fontSize: '1.1rem', fontWeight: 700, margin: '0 0 6px' }}>
                            Send {confirmGift.name}?
                        </h3>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem', margin: '0 0 22px' }}>
                        ...
                        <div style={{ display: 'flex', gap: 10 }}>
                            <button onClick={() => setConfirmGift(null)} disabled={sending}
                                style={{
                                    flex: 1, padding: '12px', borderRadius: 12, border: '1px solid var(--border-default)',
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    ...
                    <button onClick={onClose}
                        style={{ width: 32, height: 32, borderRadius: 10, border: '1px solid var(--border-default)', background: 'var(--bg-input)', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 14 }}>
                        ✕
                    </button>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 10 }}>
                    {loading ? (
                        <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px 0', color: 'var(--text-secondary)' }}>Loading gifts...</div>
                    ) : gifts.length === 0 ? (
                        <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px 0', color: 'var(--text-secondary)' }}>No gifts available</div>
                    ) : (
                        gifts.map(g => (
                            <button key={g.id} onClick={() => setConfirmGift(g)}
                                ...
                                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(212,167,44,0.12)'; e.currentTarget.style.borderColor = 'rgba(212,167,44,0.4)'; }}
                                onMouseLeave={e => { e.currentTarget.style.background = 'var(--bg-input)'; e.currentTarget.style.borderColor = 'var(--border-default)'; }}
                            >
                                <GiftVisual gift={g} />
                                <span style={{ color: 'var(--text-primary)', fontSize: '0.8rem', fontWeight: 600 }}>{g.name}</span>

...
const LEDGER_TYPE: Record<string, { label: string; color: string; bg: string }> = {
    ...
    withdraw: { label: 'Withdraw', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
    earning: { label: 'Earning', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
    event_payment: { label: 'Event Payment', color: 'var(--red-status)', bg: 'rgba(239,68,68,0.12)' },
    event_income: { label: 'Event Income', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
    membership_purchase: { label: 'Membership', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
    audio_call: { label: 'Audio Call', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
};
...
function sectionHeader(icon: React.ReactNode, title: string, action?: React.ReactNode) {
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
            <span style={{ color: 'var(--gold-mid)', display: 'flex', flexShrink: 0 }}>{icon}</span>
...
export function AdminHome() {
    ...
    const quickActions = [
        { label: 'USERS', to: '/users', icon: usersIcon, color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.1)' },
        { label: 'REPORTS', to: '/reports', icon: activityIcon, color: 'var(--green-status)', bg: 'rgba(16,185,129,0.1)' },
        { label: 'ADMIN WALLET', to: '/admin-wallet', icon: walletIcon, color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
        { label: 'GIFTS', to: '/admin-gifts', icon: <span style={{ fontSize: '14px' }}>🎁</span>, color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
        { label: 'PACKAGES', to: '/settings', icon: <span style={{ fontSize: '14px' }}>📦</span>, color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.12)' },
                    ...
                    (pending.deposits.length > 0 || pending.withdrawals.length > 0) && (
                        <button
                            ...
                            style={{
                                ...
                                color: 'var(--gold-mid)', fontSize: '0.65rem', fontWeight: 800,
                                        ...
                                        {item.kind === 'withdraw' && item.withdraw ? (
                                            <button
                                                ...
                                                style={{
                                                    ...
                                                    background: 'var(--gold-glow)', border: '1px solid rgba(212,167,44,0.3)',
                                                    color: 'var(--gold-mid)', fontSize: '0.6rem', fontWeight: 800,
                                                    letterSpacing: '0.05em', textTransform: 'uppercase', fontFamily: "'Inter', sans-serif",
                                                }}
                                            >
                                                Review
            ...
            <div className="card" style={{ padding: 'clamp(14px, 4vw, 18px)' }}>
                ...
                {loading.users ? (
                    ...
                    <>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {[
                                { label: 'Users', total: usersData?.totalUsers ?? 0, active: activeUsers, inactive: inactiveUsers, color: 'var(--gold-mid)' },
                                        ...
                                        <div key={`${r.role}-${r.name}-${r.created_at}`} style={{
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                                                <span style={{
                                                    ...
                                                    background: r.role === 'provider' ? 'rgba(212,167,44,0.15)' : 'rgba(212,167,44,0.15)',
                                                    color: r.role === 'provider' ? 'var(--gold-mid)' : 'var(--gold-mid)',

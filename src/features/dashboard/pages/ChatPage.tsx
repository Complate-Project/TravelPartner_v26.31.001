...
/* ─── helpers ─── */
const GRAD = ['135deg,#6366f1,#8b5cf6', '135deg,#0ea5e9,#06b6d4', '135deg,#ec4899,#f472b6', '135deg,#10b981,#34d399', '135deg,#D4A72C,#fbbf24', '135deg,#ef4444,#f97316'];
    ...
    return (
        <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            {online && <span style={{
                ...
                background: '#22c55e', border: `${size > 36 ? 2 : 1.5}px solid var(--bg-main)`,
                ...
                <div style={{
                    ...
                    {!selected ? (
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, animation: 'slideUp .3s ease' }}>
                            {/* Header */}
                            <div style={{ padding: '20px 18px 0', background: 'linear-gradient(180deg,var(--bg-main) 70%,transparent)', flexShrink: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                                    <div>
                                        <h1 style={{ color: 'var(--text-primary)', fontSize: '1.55rem', fontWeight: 800, letterSpacing: '-0.03em', margin: 0 }}>Messages</h1>
                                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', marginTop: 2 }}>{filtered.length} {isProvider ? 'users' : 'providers'} available</p>
                                    </div>
                                    {/* history + compose */}
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <button
                                            ...
                                            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(212,167,44,0.15)'; e.currentTarget.style.borderColor = 'rgba(212,167,44,0.3)'; }}
                                        ...
                                        <div style={{ width: 38, height: 38, borderRadius: 12, background: 'linear-gradient(135deg,#A9852A,#D4A72C)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 14px rgba(212,167,44,0.5)', cursor: 'pointer' }}>
                                ...
                                <div style={{ position: 'relative', marginBottom: 14 }}>
                                    <svg style={{ position: 'absolute', left: 13, top: '50%', transform: 'translateY(-50%)', opacity: 0.6, pointerEvents: 'none' }} width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2.2"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
                                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…"
                                        style={{ width: '100%', boxSizing: 'border-box', background: 'var(--bg-input)', border: '1px solid var(--border-default)', borderRadius: 14, color: 'var(--text-primary)', padding: '11px 14px 11px 38px', fontSize: '0.88rem', outline: 'none', fontFamily: "'Inter',sans-serif", backdropFilter: 'blur(8px)' }} />
                                </div>
                                <p style={{ fontSize: '0.65rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.12em', marginBottom: 6 }}>ONLINE NOW</p>
                            </div>
                            ...
                            <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '0 10px 8px' }}>
                                {cLoading ? (
                                    <div style={{ textAlign: 'center', padding: '50px 0' }}>
                                        {[0, 1, 2].map(i => (
                                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 8px', marginBottom: 4 }}>
                                                <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--bg-input)', animation: 'pulse 1.4s ease infinite' }} />
                                                <div style={{ flex: 1 }}>
                                                    <div style={{ height: 12, borderRadius: 6, background: 'var(--border-default)', marginBottom: 8, width: '55%', animation: 'pulse 1.4s ease infinite' }} />
                                                    <div style={{ height: 10, borderRadius: 6, background: 'var(--border-subtle)', width: '35%', animation: 'pulse 1.4s ease infinite' }} />
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', gap: 10 }}>
                                        <div style={{ width: 64, height: 64, borderRadius: 20, background: 'rgba(212,167,44,0.1)', border: '1px solid rgba(212,167,44,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(212,167,44,0.6)" strokeWidth="1.8"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
                                        </div>
                                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', textAlign: 'center' }}>No contacts online right now</p>
                                    </div>
                                ) : filtered.map((c, idx) => {
                                    ...
                                    >
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            {preview ? (
                                                <p style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: unread > 0 ? 700 : 400 }}>{preview}</p>
                                            ) : (
                                            <p style={{ fontSize: '0.72rem', color: c.is_online === 1 ? '#22c55e' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4 }}>
                                                {c.request_status === 'pending'
                                                    ? <><span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: '#D4A72C', boxShadow: '0 0 6px #D4A72C' }} />Request pending</>
                                        ...
                                        {unread > 0 ? (
                                            <span style={{ minWidth: 20, height: 20, borderRadius: 999, background: 'linear-gradient(135deg,#A9852A,#D4A72C)', color: '#fff', fontSize: '0.65rem', fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 5px', flexShrink: 0, boxShadow: '0 0 10px rgba(212,167,44,0.6)' }}>{unread > 99 ? '99+' : unread}</span>
                                        ) : c.request_status === 'pending' ? (
                                            <span style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.04em', color: '#D4A72C', background: 'rgba(212,167,44,0.12)', border: '1px solid rgba(212,167,44,0.3)', padding: '3px 8px', borderRadius: 999, flexShrink: 0 }}>PENDING</span>
                                        ) : null}
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--border-default)" strokeWidth="2.5"><polyline points="9 18 15 12 9 6" /></svg>
                                    </button>
                                    );
                                })}
                            </div>
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, animation: 'slideUp .25s ease' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--bg-nav)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border-default)', flexShrink: 0 }}>
                                ...
                                {/* info icon */}
                                <div style={{ width: 34, height: 34, borderRadius: 10, border: '1px solid var(--border-default)', background: 'var(--bg-input)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
                                </div>
                            </div>

                            {/* messages area */}
                            <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 4, background: 'radial-gradient(ellipse at top,rgba(212,167,44,0.04) 0%,transparent 60%), var(--bg-main)' }}>
                                {mLoading ? (
                                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <div style={{ display: 'flex', gap: 6 }}>
                                            {[0, 1, 2].map(i => <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: 'rgba(212,167,44,0.6)', animation: `chatDot 1.2s ease ${i * 0.2}s infinite` }} />)}
                                        </div>
                                    </div>
                                ) : messages.length === 0 ? (
                                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                                        <div style={{ width: 72, height: 72, borderRadius: 24, background: `linear-gradient(135deg, rgba(212,167,44,0.15), rgba(212,167,44,0.05))`, border: '1px solid rgba(212,167,44,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(212,167,44,0.7)" strokeWidth="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
                                        </div>
                                        <div style={{ textAlign: 'center' }}>
                                            <p style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '0.9rem', marginBottom: 4 }}>Start a conversation</p>
                                            <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>Send a message to {selected.name}</p>
                                        </div>
                                    </div>
                                ) : (
                                    messages.map((msg, i) => {
                                            ...
                                            <div key={msg.id}>
                                                {newDay && (
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '12px 0 8px' }}>
                                                            ...
                                                            <span style={{ fontSize: '0.62rem', color: 'var(--text-secondary)', fontWeight: 600, letterSpacing: '0.06em' }}>{fmtDate(msg.created_at).toUpperCase()}</span>
                                                            <div style={{ flex: 1, height: 1, background: 'var(--border-subtle)' }} />
                                                        </div>
                                                )}
                                                <div style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 8, marginBottom: clump ? 2 : 6, animation: 'msgIn .25s ease' }}>
                                                    ...
                                                    <div style={{ width: 34, flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                                                        {isLastInClump && (
                                                            <>
                                                                ...
                                                                <span style={{ fontSize: '0.55rem', fontWeight: 700, color: isMe ? 'var(--gold-mid)' : 'var(--text-secondary)', letterSpacing: '0.05em' }}>
                                                    ...
                                                    <div style={{ maxWidth: '72%', display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                                                        {gift ? (
                                                            <div style={{
                                                                background: isMe ? 'linear-gradient(135deg, var(--gold-deep), var(--gold-mid))' : 'var(--bg-input)',
                                                                backdropFilter: !isMe ? 'blur(10px)' : undefined,
                                                                border: !isMe ? '1px solid var(--border-subtle)' : 'none',
                                                                borderRadius: isMe ? (clump ? '18px 4px 4px 18px' : '18px 4px 18px 18px') : (clump ? '4px 18px 18px 4px' : '4px 18px 18px 18px'),
                                                                padding: '10px 14px',
                                                                boxShadow: isMe ? '0 4px 16px rgba(212,167,44,0.4)' : '0 1px 6px rgba(0,0,0,0.3)',
                                                                minWidth: 120,
                                                                ...
                                                                <p style={{ color: isMe ? 'rgba(26,26,26,0.75)' : 'var(--text-muted)', fontSize: '0.75rem', fontWeight: 700, margin: 0 }}>
                                                            ...
                                                            <div style={{
                                                                ...
                                                                <p style={{ color: isMe ? '#1a1a1a' : 'var(--text-primary)', fontSize: '0.9rem', lineHeight: 1.55, margin: 0, wordBreak: 'break-word' }}>{msg.message}</p>
                                                            </div>
                                                        )}
                                                        <span style={{ fontSize: '0.6rem', color: 'var(--text-muted)', marginTop: 3, paddingLeft: 4, paddingRight: 4 }}>
                                ...
                                {showTyping && (
                                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
                                        <div style={{ width: 30 }} />
                                        <div style={{ background: 'var(--bg-input)', border: '1px solid var(--border-subtle)', borderRadius: '4px 18px 18px 18px' }}>
                                            <TypingDots />
                                        </div>
                                    </div>
                                )}
                            ...
                            <div style={{ padding: '10px 12px', background: 'var(--bg-nav)', backdropFilter: 'blur(20px)', borderTop: '1px solid var(--border-default)', flexShrink: 0 }}>
                                {partnerLocked ? (
                                    <div style={{
                                        ...
                                        background: 'rgba(212,167,44,0.06)', border: '1px dashed rgba(212,167,44,0.3)',
                                    }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, background: 'var(--bg-input)', border: '1px solid var(--border-default)', borderRadius: 20, padding: '6px 6px 6px 14px', transition: 'border-color .2s' }}
                                        onFocusCapture={e => (e.currentTarget.style.borderColor = 'rgba(212,167,44,0.5)')}
                                    ...
                                    >
                                        ...
                                        {/* emoji placeholder */}
                                        <button style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '1.1rem', padding: '2px 0', lineHeight: 1, flexShrink: 0 }}>😊</button>
                                        <textarea ref={textRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={onKey}
                                            placeholder="Message…" rows={1}
                                            style={{ flex: 1, background: 'transparent', border: 'none', color: 'var(--text-primary)', resize: 'none', outline: 'none', fontFamily: "'Inter',sans-serif", fontSize: '0.9rem', lineHeight: 1.5, maxHeight: 110, overflowY: 'auto', paddingTop: 4, paddingBottom: 4 }}
                                        />
                                        {/* send btn */}
                                        <button onClick={send} disabled={!input.trim() || sending}
                                            style={{ width: 38, height: 38, borderRadius: '50%', border: 'none', flexShrink: 0, cursor: !input.trim() || sending ? 'not-allowed' : 'pointer', background: input.trim() ? 'linear-gradient(135deg,#A9852A,#D4A72C)' : 'rgba(212,167,44,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .2s', boxShadow: input.trim() ? '0 0 16px rgba(212,167,44,0.5)' : 'none' }}>

...
const typeLabel: Record<string, { label: string; color: string; bg: string }> = {
    ...
    withdraw: { label: 'Withdraw', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.1)' },
    earning: { label: 'Earning', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.1)' },
    event_payment: { label: 'Event Payment', color: 'var(--red-status)', bg: 'rgba(239,68,68,0.1)' },
    event_income: { label: 'Event Income', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.1)' },
    membership_purchase: { label: 'Membership', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.1)' },
    audio_call: { label: 'Audio Call', color: 'var(--gold-mid)', bg: 'rgba(212,167,44,0.1)' },
};
        ...
        <motion.div initial="hidden" animate="visible" variants={containerAnim} className="container" style={{ paddingBottom: '100px', paddingTop: '108px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 'var(--space-4)', marginBottom: 'var(--space-8)' }}>
                {[
                    ...
                    { label: 'Total Earnings', amount: s.totalEarnings, color: 'var(--gold-mid)', icon: icons.earning },
            ...
            <motion.div variants={fadeUp} className="card" style={{ marginBottom: 'var(--space-8)', padding: 'var(--space-6)' }}>
                ...
                {pendingMessage ? <div style={{ marginBottom: 'var(--space-3)', padding: '8px 12px', borderRadius: 'var(--radius-md)', background: 'var(--bg-card-hover)', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>{pendingMessage}</div> : null}
                    ...
                    {pendingRequests.withdrawals.map((req) => (
                        <div key={`withdraw-${req.id}`} style={{ border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-4)', background: 'var(--bg-input)' }}>
                            ...
                            <button
                                ...
                                style={{ background: 'var(--gold-glow)', color: 'var(--gold-mid)', border: '1px solid rgba(212,167,44,0.3)' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-4)', marginBottom: 'var(--space-8)' }}>
                ...
                <motion.div variants={fadeUp} className="card" style={{ padding: 'var(--space-6)' }}>
                    <h2 style={{ margin: '0 0 var(--space-4)', fontSize: '1.125rem', fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'var(--font-display)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                        <span style={{ color: 'var(--gold-mid)' }}>{icons.topEarners}</span> <span>Top Earners</span>
                    </h2>
                    {data!.topEarners.length === 0
                        ...
                        : <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                            {data!.topEarners.map((u, i) => (
                                <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', padding: 'var(--space-2)', background: 'var(--bg-input)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
                                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: i === 0 ? 'linear-gradient(135deg, var(--gold-deep), var(--gold-mid))' : 'var(--bg-card-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 700, color: '#fff', flexShrink: 0 }}>
                                        {i + 1}
                                    </div>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontSize: '0.85rem', fontWeight: 500, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.name}</div>
                                    ...
                                    <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--gold-mid)', flexShrink: 0 }}><PointsDisplay amount={u.total_earned ?? 0} decimals={2} /></div>
                                </div>
                            ))}
                        </div>
                    }
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                                                        ...
                                                        <span
                                                            ...
                                                            style={{
                                                                background: e.user_role === 'admin' ? 'var(--gold-glow)' : e.user_role === 'provider' ? 'var(--gold-glow)' : 'rgba(16,185,129,0.1)',
                                                                color: e.user_role === 'admin' ? 'var(--gold-mid)' : e.user_role === 'provider' ? 'var(--gold-mid)' : 'var(--green-status)',
                                                                borderColor: e.user_role === 'admin' ? 'var(--border-gold)' : e.user_role === 'provider' ? 'rgba(212,167,44,0.3)' : 'rgba(16,185,129,0.3)',
                                                                fontWeight: 800,
                                                            }}
                                                        >
                                                            {e.user_role}
            ...
            >
                <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                    <button
                        ...
                        style={{
                            alignSelf: 'flex-end',
                            background: 'rgba(203,213,235,0.2)',
                            border: '1px solid rgba(203,213,235,0.35)',
                            color: '#fff',
                            width: 36,
                            height: 36,
                            borderRadius: 999,
                    ...
                    <div
                        style={{
                            ...
                            borderRadius: 'var(--radius-lg)',
                            overflow: 'hidden',
                            background: 'rgba(203,213,235,0.14)',
                            border: '1px solid rgba(203,213,235,0.25)',
                            padding: 8,
                        }}
                    >

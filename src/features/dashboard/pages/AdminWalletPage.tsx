...
function typeInfo(type: string): { label: string; bg: string; color: string; border: string } {
    switch (type) {
        case 'membership_income':
            return { label: 'Membership', bg: 'var(--gold-glow)', color: 'var(--gold-mid)', border: 'var(--border-default)' };
        case 'audio_call_income':
            return { label: 'Call Income', bg: 'var(--gold-glow)', color: 'var(--gold-mid)', border: 'rgba(212,167,44,0.3)' };
        case 'withdraw':
...
export function AdminWalletPage() {
    ...
    const statCards = data ? [
        ...
        { label: 'Call Income', amount: data.totalCallIncome, color: 'var(--gold-mid)', icon: icons.call },

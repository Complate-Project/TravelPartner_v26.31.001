                                            ...
                                            return (
                                                <button
                                                    ...
                                                    style={{
                                                        ...
                                                        background: selected ? 'rgba(212,167,44,0.1)' : 'var(--bg-input)',
                                                        border: selected ? '2px solid var(--gold-mid)' : '1px solid var(--border-subtle)',
                                                        boxSizing: 'border-box',
                                                    }}
                                                >
                                                    <div style={{ width: 44, height: 44 }}>{assetThumb(a)}</div>
                                                    <span style={{ fontSize: '0.58rem', color: selected ? 'var(--gold-mid)' : 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{a.name}</span>
                                                </button>
                                            );
                                        })}
                                    </div>
                            ...
                            {gifts.map(g => (
                                <div key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', padding: 'var(--space-3) 0', borderBottom: '1px solid var(--border-subtle)' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 'var(--space-2)', flexShrink: 0 }}>
                                        ...
                                        {g.is_active === 1 ? (
                                            <button className="btn btn-ghost btn-sm" onClick={() => handleToggle(g)} disabled={togglingId !== null} style={{ padding: '6px 12px', color: '#D4A72C', borderColor: 'rgba(212,167,44,0.4)' }}>
                                                {togglingId === g.id ? <MiniSpinner color="#D4A72C" /> : 'Deactivate'}
                                            </button>
                                        ) : (
